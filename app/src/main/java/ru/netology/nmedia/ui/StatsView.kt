...
class StatsView @JvmOverloads constructor(
    ...
    //    private val paintForArcs = paint
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val newData: List<Float> = data.subList(0, data.size - 2)


        val dataSum = data.sum()
        val sweepAngleForFirstElement = (360 * (data[0] / dataSum)) / 5
        var actualProcents = 0F
        var startFrom = -90F
        ...
        if (newData.size == data.size) {
            canvas.drawArc(oval, startFrom, sweepAngleForFirstElement, false, paintForFirstArc)
        }

        canvas.drawText(
            "%.2f%%".format(actualProcents * 100),
            center.x,
